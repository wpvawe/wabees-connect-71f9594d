...
import { faCheck, faCheckDouble, faClock, faTriangleExclamation } from "@fortawesome/free-solid-svg-icons";
        ...
        {m.mediaUrl && (
          <div className="mb-1 overflow-hidden rounded-md">
            {m.mimeType?.startsWith("image/") ? (
              ...
              <img src={m.mediaUrl} alt={m.caption ?? "image"} className="max-h-64 w-auto" loading="lazy" />
            ) : (
...
function StatusIcon({ status }: { status: string }) {
  ...
  if (status === "delivered") return <FontAwesomeIcon icon={faCheckDouble} className="h-3 w-3 opacity-70" />;
  if (status === "failed") return <FontAwesomeIcon icon={faTriangleExclamation} className="h-3 w-3" />;
