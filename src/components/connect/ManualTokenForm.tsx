...
export function ManualTokenForm() {
  ...
  const m = useMutation({
    mutationFn: async (v: ManualConnectValues) => {
      ...
      // Use the same PHP backend the mobile app uses. Direct browser → Meta
      // calls are blocked for some fields and diverge from the app behavior.
      let phone: PhoneInfo = {};
  ...
  return (
    <form onSubmit={handleSubmit((v) => m.mutate(v))} className="space-y-5">
