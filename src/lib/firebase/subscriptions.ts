...
export async function requestSubscription(uid: string, plan: Plan): Promise<void> {
  ...
  const currentSnap = await getDoc(doc(db, "users", uid, "subscription", "current")).catch(() => null);
  const current = currentSnap?.exists() ? (currentSnap.data() as Record<string, unknown>) : {};
  await setDoc(doc(db, "pending_subscriptions", uid), {
    planId: plan.id,
    planName: plan.name,
    status: "pending",
    userId: uid,
    userName: (user.businessName as string | undefined) ?? (fbAuth().currentUser?.displayName ?? ""),
