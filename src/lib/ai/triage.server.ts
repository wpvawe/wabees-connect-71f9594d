/**
 * Server-only classifier used by both:
 *   - createServerFn `classifyMessage` (web callers)
 *   - server route `/api/public/triage-message` (Flutter app callers)
 *
 * Verifies the caller's Firebase idToken, then proxies DeepSeek. Same
 * prompt / output shape / tag allowlist as the original TanStack server
 * function so both surfaces produce identical results.
 */
...
export function parseTriageInput(raw: unknown): TriageInput {
...
function normalizePriority(v: unknown): TriageOutput["priority"] {
  if (v === "urgent" || v === "high" || v === "normal" || v === "low") return v;
  return "normal";
}

function normalizeSentiment(v: unknown): TriageOutput["sentiment"] {
...
export async function runTriage(input: TriageInput): Promise<TriageOutput> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 429)
      throw new Error("DeepSeek rate limit reached. Try again in a moment.");
  ...
  const allowed = new Set(input.categories.map((c) => c.toLowerCase()));
  ...
  const tags = tagsRaw
    ...
    .map(
      (t) =>
        input.categories.find((c) => c.toLowerCase() === t.toLowerCase()) ?? t,
    )
    .slice(0, 4);
