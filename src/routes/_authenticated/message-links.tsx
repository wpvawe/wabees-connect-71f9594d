...
function MessageLinksPage() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, (snap) => {
      setLinks(snap.docs.map((d) => {
        const x = d.data() as Record<string, unknown>;
        return { id: d.id, message: str(x.message), url: str(x.url), createdAt: toIso(x.createdAt) };
      }));
    });
  }, [uid]);

  const phoneDigits = (wa?.display_phone ?? "").replace(/[^0-9]/g, "");
  const previewUrl = phoneDigits && message.trim()
  ...
  async function remove(id: string) {
    if (!uid) return;
    try { await deleteDoc(doc(fbDb(), `users/${uid}/message_links/${id}`)); }
    ...
    return (
      <>
        ...
        <div className="px-4 py-6 sm:px-6">
          <WbEmpty icon={faPlug} title="Connect WhatsApp first" description="Link your business number from Connect to create message links." />
        </div>
      </>
    );
  }
  ...
  return (
    <>
      <TopBar title="Message Links" subtitle="Create shareable wa.me links with prefilled messages" />
      <div className="mx-auto w-full max-w-2xl space-y-4 px-4 py-6 sm:px-6">
        <WbCard>
          ...
          <WbCardBody className="space-y-3">
            ...
            />
            {previewUrl && <p className="break-all rounded bg-muted px-2 py-1.5 text-xs text-muted-foreground">{previewUrl}</p>}
            <div className="flex justify-end">
              <WbButton onClick={create} loading={saving} disabled={!message.trim()}>Create link</WbButton>
            </div>
          </WbCardBody>
        </WbCard>
        ...
        {links === null ? (
          ...
          <WbEmpty icon={faLink} title="No links yet" description="Create your first prefilled wa.me link above." />
        ) : (
          <ul className="space-y-2">
            {links.map((l) => (
              <li key={l.id} className="rounded-xl border border-border bg-card p-3">
                ...
                <a href={l.url} target="_blank" rel="noreferrer" className="mt-1 block break-all text-xs text-primary underline">{l.url}</a>
                <div className="mt-2 flex items-center justify-between gap-2">
                  <span className="text-[10px] text-muted-foreground">{l.createdAt ? format(new Date(l.createdAt), "PPp") : ""}</span>
                  <div className="flex gap-1">
                    <WbButton size="sm" variant="ghost" onClick={() => copyUrl(l.url)}><FontAwesomeIcon icon={faCopy} className="h-3.5 w-3.5" /> Copy</WbButton>
                    <WbButton size="sm" variant="ghost" onClick={() => remove(l.id)}><FontAwesomeIcon icon={faTrash} className="h-3.5 w-3.5 text-destructive" /></WbButton>
                  </div>
                </div>
              </li>
            ))}
