    ...
    return (
      <>
        ...
        <div className="px-4 py-6 sm:px-6">
          <WbEmpty icon={faPlug} title="Connect WhatsApp first" description="Insights are fetched from Meta for your connected number."
    ...
    <>
      ...
      <div className="space-y-5 px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-center gap-2">
          {RANGES.map((r) => (
            <button key={r.id} onClick={() => setRange(r.id)} className={cn("h-9 rounded-md border px-3 text-sm transition-colors", range === r.id ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-foreground hover:bg-muted")}>{r.label}</button>
          ))}
        </div>
        ...
        {error ? (
          <WbCard><WbCardBody className="space-y-3">
            ...
            <WbButton size="sm" variant="secondary" onClick={reload}>Retry</WbButton>
          </WbCardBody></WbCard>
        ) : loading || !data ? (
